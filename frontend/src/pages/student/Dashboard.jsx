        ...
        <div className="fixed inset-0 z-50 flex md:hidden">
          ...
          <aside className="w-64 bg-slate-900 text-slate-300 flex flex-col transition-all duration-300 relative z-50 h-full">
            ...
            <nav className="flex-1 px-4 space-y-2 mt-4 overflow-y-auto">
              ...
              <button onClick={() => {setActiveTab('results'); setIsMobileMenuOpen(false);}} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'results' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'hover:bg-slate-800 hover:text-white'}`}>
                <TrendingUp className="w-5 h-5" />
                <span className="font-medium text-sm">My Results</span>
              </button>
              <button onClick={() => {setActiveTab('materials'); setIsMobileMenuOpen(false);}} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'materials' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'hover:bg-slate-800 hover:text-white'}`}>
                <BookOpen className="w-5 h-5" />
              ...
              <button onClick={() => {setActiveTab('resumes'); setIsMobileMenuOpen(false);}} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'resumes' ? 'bg-secondary/10 border-l-4 border-secondary text-secondary-container font-bold' : 'hover:bg-slate-800 hover:text-white border-l-4 border-transparent'}`}>
                <FileBadge className="w-5 h-5" />
                <span className="text-sm">My Saved Resumes</span>
              </button>
              <button onClick={() => {setActiveTab('roadmap'); setIsMobileMenuOpen(false);}} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'roadmap' ? 'bg-secondary/10 border-l-4 border-secondary text-secondary-container font-bold' : 'hover:bg-slate-800 hover:text-white border-l-4 border-transparent'}`}>
                <Map className="w-5 h-5 text-secondary" />
                <span className="text-sm text-secondary">Career Roadmap</span>
              </button>
            </nav>
      ...
      <main className="flex-1 flex flex-col overflow-y-auto scroll-smooth">
        ...
        <header className="h-20 glass-card border-b border-white/20 flex items-center justify-between px-4 md:px-8 sticky top-0 z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl md:text-2xl font-headline-md text-slate-900 font-semibold truncate max-w-[200px] md:max-w-none">Hello, {currentUser?.email?.split('@')[0] || 'Student'}</h2>
              <p className="text-xs md:text-sm text-slate-500 hidden sm:block">Ready to bridge the gap in your learning today?</p>
            </div>
          </div>
                ...
                {userProfile?.results && userProfile.results.length > 0 && (
                  <div className="flex items-center gap-4 bg-slate-50 px-4 py-3 rounded-2xl border border-slate-100">
                    <div className="text-center">
                      ...
                      {(() => {
                        const totalMarks = userProfile.results.reduce((s, r) => s + (typeof r.mark === 'number' ? r.mark : 0), 0);
                        const totalMax = userProfile.results.reduce((s, r) => s + r.max, 0);
                        const percent = totalMax > 0 ? (totalMarks / totalMax) * 100 : 0;
                        const isFailed = percent < 50 || userProfile.results.some(r => !r.isPass);
                        return isFailed ? (
